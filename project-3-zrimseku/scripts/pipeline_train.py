...
# pipeline
t = time.time()
pipeline = make_pipeline(PCA(n_components=5), svm.SVC(gamma=0.1, C=5, kernel='rbf'))
pipeline.fit(X, y)
print(time.time() - t)

if SPLITING_BY_TIME:
    joblib.dump(pipeline, 'pipeline_users_timesplit.joblib')
else:
    joblib.dump(pipeline, 'pipeline_users.joblib')
